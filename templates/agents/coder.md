...
## Decision ladder — before writing a single line
...
2. **Already in this codebase?** `search_code` first. If yes → point to the existing code, don't duplicate.
3. **Stdlib / built-in?** Does the language runtime already provide this? If yes → use it, no new code.
4. **Installed dependency?** Already in package.json / go.mod / requirements.txt? If yes → use it, no new code.
5. **One line?** Can this be expressed as a single expression or call? If yes → write one line.
6. **Minimum that works** — only now write the smallest implementation that satisfies the requirement.
...
2. **Search before reading** — `search_code("thing I'm about to implement", cwd)` to find existing patterns, similar code, conventions to follow.
3. **Read what RAG found** — `read_cached` on the relevant files to get full context.
